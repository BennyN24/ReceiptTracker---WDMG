...
import type { Category, ExtractedReceiptData } from '../types';
...
const CaptureScreen: React.FC<CaptureScreenProps> = ({ navigation }) => {
  ...
  const fallbackToOCR = useCallback(async (imageUri: string) => {
    setIsProcessing(true);
    setProcessingStage('Falling back to OCR...');
    try {
      const result = await OCRService.extractReceiptData(imageUri);
      if (result) {
        setOcrData(result as unknown as ExtractedReceiptData);
  ...
  const processImage = useCallback(async () => {
    ...
    try {
      ...
      if (geminiResult && !geminiResult.error) {
        const quality = GeminiService.getExtractionQuality(geminiResult as unknown as ExtractedReceiptData);
        setOcrData(geminiResult as unknown as ExtractedReceiptData);
        setProcessingStage('');
