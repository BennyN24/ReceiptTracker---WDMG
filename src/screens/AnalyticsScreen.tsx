...
const AnalyticsScreen: React.FC = () => {
  ...
  const getDateRange = useCallback((periodType: PeriodType) => {
    const now = new Date();
  ...
  const renderStatCard = useCallback((label: string, value: number, color: string) => (
  ...
  return (
    ...
    >
      ...
      <HStack px="$4" py="$3" space="sm">
        {(['week', 'month', 'quarter', 'year'] as PeriodType[]).map((item) => (
          <Pressable
            key={item}
            flex={1}
            onPress={() => setPeriod(item)}
            bg={period === item ? colors.primary : colors.white}
            borderWidth={1}
            borderColor={period === item ? colors.primary : colors.border}
            borderRadius="$lg"
            py="$2.5"
            alignItems="center"
          >
            <Text
              fontSize="$sm"
              fontWeight="$medium"
              color={period === item ? colors.white : colors.textSecondary}
            >
