...
import type { Budget, Expense, Category, AppSettings, BudgetPeriod } from '../types';
...
const BudgetsScreen: React.FC<BudgetsScreenProps> = ({ navigation }) => {
  ...
  const getCurrentPeriodExpenses = useCallback((period: BudgetPeriod) => {
    const now = new Date();
  ...
  const getBudgetSpent = useCallback((budget: Budget): number => {
    const periodExpenses = getCurrentPeriodExpenses(budget.period);
  ...
  const getBudgetPercentage = useCallback((budget: Budget): number => {
  ...
  const handleAddBudget = async (budgetData: Omit<Budget, 'id' | 'createdAt'>): Promise<void> => {
    try {
      if (editingBudget) {
  ...
  const formatCurrency = useCallback((amount: number, currency: string | null = null): string => {
  ...
  const getCategoryForBudget = useCallback((budget: Budget): Category | undefined => {
    return categories.find((c) => c.id === budget.categoryId);
  }, [categories]);

  const renderBudgetItem = useCallback((budget: Budget) => {
    const spent = getBudgetSpent(budget);
  ...
  }, [getBudgetSpent, getBudgetPercentage, getProgressColor, formatCurrency, getCategoryForBudget, handleEditBudget, handleDeleteBudget, colors]);
  ...
  return (
    <Box flex={1} bg={colors.backgroundSecondary} pt="$8">
      ...
      <Pressable
        ...
        // @ts-ignore - style and shadow props
