...
const CurrencySettingsScreen: React.FC = () => {
  ...
  const loadCurrencySettings = useCallback(async (): Promise<void> => {
    try {
      setLoading(true);
      ...
      const currencyList: CurrencyInfo[] = CurrencyService.getAllCurrencies();
      setCurrencies(currencyList);

      const settings = await StorageService.getSettings();
      setAutoDetect(settings.autoDetectCurrency ?? true);
      setDetectedCountry((settings as any).detectedCountryCode || null);
    } catch (error) {
  ...
  const handleCurrencyChange = useCallback(async (currencyCode: string): Promise<void> => {
    try {
  ...
  const handleAutoDetectToggle = useCallback(async (value: boolean): Promise<void> => {
    try {
      setAutoDetect(value);
      const settings = await StorageService.getSettings();
      await StorageService.saveSettings({
        ...
        locationDetectionAttempted: value ? false : (settings as any).locationDetectionAttempted,
      });
    } catch (error) {
  ...
  const handleDetectNow = useCallback(async (): Promise<void> => {
    try {
      setDetecting(true);
  ...
  const renderCurrencyItem = useCallback(({ item }: ListRenderItemInfo<CurrencyInfo>) => (
    <Pressable
    ...
    >
      ...
      <VStack px="$4" py="$3">
        ...
        <FlatList
          ...
          keyExtractor={(item: CurrencyInfo) => item.code}
          scrollEnabled={false}
        />
      </VStack>
          ...
          {currencies.slice(0, 5).map((currency) => (
            <HStack key={currency.code} justifyContent="space-between" py="$1.5" borderBottomWidth={1} borderBottomColor={colors.border}>
              ...
              <Text fontSize="$sm" color={colors.textSecondary}>
                1 USD = {(1 / (currency.rate || 1)).toFixed(4)} {currency.code}
              </Text>
            </HStack>
          ))}
        </VStack>
