...
import type { RecurringExpense, FrequencyOption } from '../types';
...
const RecurringExpensesScreen: React.FC = () => {
  ...
  const frequencyOptions: FrequencyOption[] = RecurringExpenseService.getFrequencyOptions();
  ...
  const handleAddExpense = async (): Promise<void> => {
    try {
      ...
      const expenseData: Omit<RecurringExpense, 'id' | 'nextDueDate'> = {
  ...
  const handleEditExpense = (expense: RecurringExpense): void => {
    setFormData({
  ...
  const renderExpenseItem = useCallback(({ item }: ListRenderItemInfo<RecurringExpense>) => (
