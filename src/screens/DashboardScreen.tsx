...
const DashboardScreen: React.FC<DashboardScreenProps> = ({ navigation }) => {
  ...
  const getCurrentMonthExpenses = useMemo((): Expense[] => {
    const now = new Date();
  ...
  const getBudgetPercentage = useMemo((): number => {
  ...
  const getRecentExpenses = useMemo((): Expense[] => {
    return [...expenses]
  ...
  const categoryChartData = useMemo((): CategoryChartItem[] => {
    ...
    const total = byCategory.reduce((sum: number, item: any) => sum + item.amount, 0);

    return byCategory.slice(0, 6).map((item: any, index: number) => {
  ...
  const getDailyTrendData = useMemo((): ChartDataset | null => {
    const now = new Date();
  ...
  const getWeeklyTrendData = useMemo((): ChartDataset | null => {
    const now = new Date();
    const last4Weeks: { label: string; total: number }[] = [];
  ...
  const monthlyTrendData = useMemo((): ChartDataset | null => {
    const comparison = AnalyticsService.getMonthOverMonthComparison(expenses, 6);
    if (comparison.length === 0) return null;

    return {
      labels: comparison.map((item: any) => item.month.split(' ')[0]),
      datasets: [
        {
          data: comparison.map((item: any) => item.total || 0),
        },
      ],
    };
  }, [expenses]);

  const currentTrendData = useMemo((): ChartDataset | null => {
    switch (chartPeriod) {
      case 'daily':
