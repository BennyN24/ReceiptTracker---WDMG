...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ navigation }) => {
  ...
  const updateSetting = useCallback(async (key: string, value: unknown): Promise<void> => {
    try {
      const updatedSettings: AppSettings = { ...settings, [key]: value };
  ...
  const handleCurrencyChange = async (currencyCode: string): Promise<void> => {
    try {
  ...
  const performExport = async (format: 'json' | 'csv'): Promise<void> => {
    ...
    try {
      ...
      const errMsg = error instanceof Error ? error.message : 'Failed to export data';
      Toast.show({
        type: 'error',
        text1: 'Export Failed',
        text2: errMsg,
      ...
      >
        <Box flex={1} bg="rgba(0, 0, 0, 0.5)" justifyContent="flex-end">
          <Box bg={colors.white} borderTopLeftRadius="$2xl" borderTopRightRadius="$2xl" maxHeight="80%" pt="$4">
            ...
            <FlatList
              data={currencies}
              renderItem={({ item }: ListRenderItemInfo<CurrencyInfo>) => (
                <Pressable
