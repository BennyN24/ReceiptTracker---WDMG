...
import type { Expense, Frequency, RecurringExpense, RecurringExpenseData } from '../types';

const RECURRING_EXPENSES_KEY = '@receipt_tracker_recurring_expenses';

// ─── StorageService interface (to break circular import) ──
...
interface StoredRecurringExpense extends RecurringExpense {
  endDate?: string | null;
...
interface ImportRecurringResult {
  success: boolean;
  imported: number;
}
...
const RecurringExpenseService = {
   ...
   */
  async createRecurringExpense(recurringExpense: RecurringExpenseData & { endDate?: string | null; receiptImage?: string | null }): Promise<StoredRecurringExpense> {
    try {
      const id = await generateSecureId();

      const newRecurringExpense: StoredRecurringExpense = {
        id,
   ...
   */
  async getRecurringExpenses(): Promise<StoredRecurringExpense[]> {
    try {
   ...
   */
  async getActiveRecurringExpenses(): Promise<StoredRecurringExpense[]> {
    try {
      const expenses = await this.getRecurringExpenses();
      const now = new Date();
   ...
   */
  async updateRecurringExpense(id: string, updates: Partial<StoredRecurringExpense>): Promise<StoredRecurringExpense> {
    try {
      const expenses = await this.getRecurringExpenses();
   ...
   */
  async getExpensesDueToday(): Promise<StoredRecurringExpense[]> {
    try {
   ...
   */
  async processDueExpenses(storageService: StorageServiceLike): Promise<Expense[]> {
    try {
      ...
      for (const recurring of dueExpenses) {
        try {
          ...
          errors.push({ recurringId: recurring.id, error: message });
        }
      }
   ...
   */
  _calculateNextDueDate(startDate: string, frequency: Frequency): string {
   ...
   */
  async getTotalRecurringForPeriod(startDate: string, endDate: string): Promise<number> {
    try {
      const active = await this.getActiveRecurringExpenses();
      let total = 0;
   ...
   */
  async importRecurringExpenses(recurringExpenses: Partial<StoredRecurringExpense>[]): Promise<ImportRecurringResult> {
    try {
      if (!Array.isArray(recurringExpenses)) {
        throw new Error('Recurring expenses must be an array');
      }

      const validExpenses = recurringExpenses.filter((expense): expense is StoredRecurringExpense => {
   ...
   */
  _countOccurrences(startDate: string, periodStart: string, periodEnd: string, frequency: Frequency): number {
