...
import type { ExtractedReceiptData, ExtractionQuality, ReceiptItem } from '../types';
...
const GeminiService = {
   ...
   */
  async analyzeReceipt(imageUri: string): Promise<ExtractedReceiptData | null> {
   ...
   */
  _parseResponse(apiResponse: GeminiApiResponse): ExtractedReceiptData | null {
    try {
      ...
      const parsed: RawParsedData = JSON.parse(text);
   ...
   */
  _sanitizeAndValidate(data: RawParsedData): ExtractedReceiptData | null {
    if (!data || typeof data !== 'object') return null;

    const sanitized: ExtractedReceiptData = {
    ...
    if (sanitized.items.length > 0) {
      sanitized.description = sanitized.items
        .map((i: ReceiptItem) => {
          ...
          line += i.name;
          if (i.price) line += ` - ${CurrencyService.getSymbol(sanitized.currency || 'USD')}${i.price.toFixed(2)}`;
          return line;
        })
        .join('\n');
    }
   ...
   */
  _mapCategory(categoryName: string | undefined): string {
   ...
   */
  _sanitizeString(str: string | undefined | null): string | null {
   ...
   */
  _sanitizeNumber(val: string | number | undefined | null): number | null {
   ...
   */
  _sanitizeDate(dateStr: string | undefined | null): string {
   ...
   */
  _sanitizeItems(items: RawParsedData['items']): ReceiptItem[] {
   ...
   */
  getExtractionQuality(data: ExtractedReceiptData | null): ExtractionQuality {
    if (!data) {
