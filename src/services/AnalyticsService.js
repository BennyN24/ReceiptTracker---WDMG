const AnalyticsService = {
   ...
   */
  getSpendingTrend(expenses, period = 'daily', startDate, endDate) {
    try {
      const filtered = this._filterByDateRange(expenses, startDate, endDate);
      const trendMap = {};
   ...
   */
  getTopVendors(expenses, limit = 10, startDate, endDate) {
    try {
      const filtered = this._filterByDateRange(expenses, startDate, endDate);
      const vendorMap = {};
   ...
   */
  getBudgetComparison(expenses, budgets, startDate, endDate) {
    try {
      const filtered = this._filterByDateRange(expenses, startDate, endDate);
   ...
   */
  getMonthOverMonthComparison(expenses, months = 3) {
    try {
      const comparison = [];
      const now = new Date();
   ...
   */
  _filterByDateRange(expenses, startDate, endDate) {
